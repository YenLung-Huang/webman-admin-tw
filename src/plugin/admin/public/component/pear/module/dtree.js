...
//			level: "level",			  //層級
    ...
    /**
     * @Desc: 設定基本屬性值
     * @Param: options：JSON對象，初始設定dtree所需的全部屬性
     */
    DTree.prototype.setting = function(options) {
        ...
        this.asyncLoad = this.options.asyncLoad || OPTIONS.asyncLoad || []; //初始非同步載入層級資料
        ...
        this.beforeSend = this.options.beforeSend || OPTIONS.beforeSend || function(ajax, XMLHttpRequest, self){return true}; //非同步載入之前的回呼
        this.success = this.options.success || OPTIONS.success || function(data, obj, first){}; //樹載入完畢後執行解析樹之前的回呼
        this.done = this.options.done || OPTIONS.done || function(data, obj, first){};	 //樹載入完畢後的回呼
    ...
    /**
     * @Desc: 重置基本屬性值
     * @Param: options：JSON對象，重設dtree所需的全部屬性
     */
    DTree.prototype.reloadSetting = function(options) {
        ...
        this.asyncLoad = this.options.asyncLoad || this.asyncLoad; //初始非同步載入層級資料
        ...
        this.beforeSend = this.options.beforeSend || this.beforeSend; //非同步載入之前的回呼
    ...
    DTree.prototype.selectSetting = function() {
        ...
        this.selectCardHeight = this.options.selectCardHeight || "350";			// 下拉麵板的高度
    ...
    DTree.prototype.reloadSelectSetting = function() {
        ...
        this.selectCardHeight = this.options.selectCardHeight || this.selectCardHeight;			// 下拉麵板的高度
    ...
    /**
     * @Desc: 設定下拉樹模式中，input輸入框的值
     * @Param: param： input中的值对应树数据的ID值\当前树選中的数据JSON对象，可不传，不传的话则树内部读取
     */
    ...
    /**
     * @Desc: 重置下拉樹的值，1.表單清空；2.節點重置；3.複選框重置
     */
    ...
    /**
     * @Desc: 賦值一級圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: fnode： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 賦值二級圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: snode： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 賦值複選框圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: checkbox： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 賦值選單欄圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: menubar： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 賦值擴充功能表列圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: menubarExt： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 賦值工具列圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: toolbar： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 賦值擴充工具列圖示的class屬性變數
     * @Param: ifont： 目前的圖示class前綴，如 dtreefont
     * @Param: toolbarExt： 目前的圖標class實際圖標，如 dtree-icon-jia1
     */
    ...
    /**
     * @Desc: 設定圖示的展開關閉，以及展開時/關閉時是最後一級圖示的處理
     * @Param: $i_fnode： JQuery對象，表示一級圖示節點
     * @Param: $i_snode： JQuery對象，表示二級圖示節點
     */
    ...
    /**
     * @Desc: 顯示樹線
     * @Param: $lis： JQuery對象，表示組成樹的li節點集
     */
    ...
    /**
     * @Desc:  重載樹
     * @Param: options： dtree的所有屬性的json物件
     */
    ...
    /**
     * @Desc:  初始化載入子節點
     * @Param: asyncLoad： 需非同步載入的層級，在基礎屬性中配置
     * @Param: i： 已經載入到的層級
     */
    DTree.prototype.loadChildTreeInit = function(asyncLoad, i){
    	var _this = this;
    	if(i == asyncLoad.length) { // 滿足條件，終止遞迴
    		return ;
    	}
    ...
    /**
     * @Desc:  載入子節點
     * @Param: $div： JQuery對象，代表目前子節點的父節點div
     * @Param: data： data方式載入的資料
     * @Param: callback： 子節點載入完成之後的回呼喊出
     */
     ...
     * @Param: $ul： JQuery對象，代表目前資料載入到的ul標籤裡
            ...
            success: function(result) {
                ...
                if (code == _this.response.statusCode) {
                    ...
                    if(first && typeof d.length === 'undefined'){
                        $ul.html(_this.getNoneDom().errText("資料解析異常，url回呼後的資料格式不正確"));
    ...
    /**
     * @Desc:  初始化渲染樹或拼接渲染樹，list模式下
     * @Param: nowListData： 表示目前需要被渲染的資料
    ...
    DTree.prototype.loadTree = function(listData, level, $ul){
        ...
        if (listData) {
            $ul = $ul || _this.getNodeDom().nowOrRootUl();	//目前選取的節點或根節點
            for (var i = 0; i < listData.length; i++) {	// 遍歷跟節點或追加的跟節點
                var data = listData[i];
                if(typeof data !== "object") continue;
     ...
     * @Desc: 新增節點的dom值
     * @Param: treeId： 節點ID
     * @Param: parentId： 節點父ID
     * @Param: title： 節點標題
     * @Param: fmtTitle： 節點附加標題
     * @Param: last： 是否最後一級節點
     * @Param: ficonClass： 一級圖示class
     * @Param: iconClass： 二級圖示class
     * @Param: checkArr： 複選框
     * @Param: spread： 是否展開
     * @Param: disabled： 是否禁用
     * @Param: hide： 是否隱藏
    ...
    /**
     * @Desc: 替換節點的dom值，或指定這些值
     * @Param: $div： JQuery對象，表示目前被操作的節點
     * @Param: treeId： 節點ID
     * @Param: last： 是否最後一級節點
     * @Param: spread： 是否展開
     * @Param: disabled： 是否禁用
     ...
     * @Param: disabled： 是否禁用
     * @Param: hide： 是否隱藏
     * @Param: basicData： 自訂的擴充字段值（轉義後的JSON字元）
     * @Param: recordData： 開啟記錄模式的記錄值（轉義後的JSON字元）
     * @Param: isRoot： 當前節點是否為根節點下的節點
     */
    ...
    /**
     * @Desc: 手風琴模式操作其他節點
     * @Param: $ul： JQuery對象，表示dtree中的ul節點
     */
    ...
    /**
     * @Desc: 展開或隱藏節點 作用點： div
     * @Param: $div： JQuery對象，表示目前操作的div節點
     */
    ...
    /**
     * @Desc: 設定節點為disabled
     * @Param: disabledIds： 需要設定禁用的節點id字串集合（用,分割）/id數組
     */
    ...
    /**
     * @Desc: 將節點的disabled取消
     * @Param: disabledIds： 需要設定取消禁用的節點id字串集合（用,分割）/id數組
     */
    ...
    /**
     * @Desc: 將節點的disabled取消
     * @Param: $div： JQuery對象，需要設定取消禁用的節點
     */
    ...
    /**
     * @Desc: 取得指定disabled節點的值
     * @Param: disabledIds： 需要取值的節點id字串集合（用,分割）/id數組
     */
    ...
    /**
     * @Desc: 設定節點為hide
     * @Param: hideIds： 需要隱藏的節點id字串集合（用,分割）/id數組
     */
    ...
    /**
     * @Desc: 將節點的hide取消
     * @Param: hideIds： 需要取消隱藏的節點id字串集合（用,分割）/id數組
     */
    ...
    /**
     * @Desc: 設定節點為hide
     * @Param: $div： JQuery對象，需要設定取消隱藏的節點
     */
    ...
    /**
     * @Desc:  取得指定hide節點的值
     * @Param: hideIds： 需要取值的節點id字串集合（用,分割）/id數組
     */
    ...
    /**
     * @Desc: 重新整理樹，1.清空樹結構，2.清空參數，3.執行初始化方法
     */
    ...
    /**
     * @Desc: 局部重新整理樹--新增子節點時
     * @Param: $div: JQuery對象，目前新增節點的父節點Div
     * @Param: data： 目前需要渲染的數據，JSON物件、JSON集合，如不填，則呼叫getChild方法呼叫伺服器載入資料
     */
    ...
    /**
     * @Desc: 局部重新整理樹--編輯目前節點選取節點時
     * @Param: $div: JQuery對象，當前編輯節點的Div
     * @Param: data： 目前需要渲染的數據，JSON物件、字串
     */
    ...
    /**
     * @Desc: 局部重新整理樹--當前節點選取被刪除時
     * @Param: $div: JQuery對象，目前被刪除節點的Div
     */
    ...
    /**
     * @Desc: 初始化複選框的值
     * @Param: chooseIds: 需要被選取的節點id字串集合（用,分割）/id數組/id二維數組（多層複選框）
     */
        ...
        if(chooseId.length) {
        	...
        	for (var i=0; i<chooseId.length; i++) {
        		...
        		if(typeof chooseIds !== "string" && choose.length) {
        			// 說明內部還是一個數組，將這個數組放入二維數組中
        			chooseIdArrs.push(choose);
        		}
        	}
        	if(chooseIdArrs.length == 0) {
        		// 說明上一步沒有收穫數組，那麼chooseId只為一個id數組
    ...
    /**
     * @Desc: 實作複選框點擊，子集選取父級也選取
     * @Param: $i: JQuery對象，表示目前點擊的checkbox節點
     */
    ...
    /**
     * @Desc: 實作複選框點擊， no-all 子集選中父級半選中，子集全選父級選中
     * @Param: $i: JQuery對象，表示目前點擊的checkbox節點
     */
    ...
    /**
     * @Desc: 實現複選框點擊，p-casc：父級選中子集全選，子集無法改變父級選中狀態
     * @Param: $i: JQuery對象，表示目前點擊的checkbox節點
     */
    ...
    /**
     * @Desc: 複選框半選狀態初始化設置，樹節點渲染完畢後重置半選狀態
     */
    ...
    /**
     * @Desc: 複選框選取狀態初始化設置，樹節點渲染完畢後重置選取狀態
     */
    ...
    /**
     * @Desc: 設定複選框選取/未選取/半選  _this.checkStatus($i).check();  _this.checkStatus($i).noCheck();   _this.checkStatus($i).noallCheck();
     * @Param: $i: JQuery對象，表示目前點擊的checkbox節點
     */
    ...
    /**
     * @Desc: 設定樹的複選框操作值的全部參數,並取得
     * @Param: requestParamFlag: 設定複選框傳回值的資料結構， true：基於請求類型的返回物件數組， false：原始的節點返回物件數組
     */ 
    ...
    /**
     * @Desc: 取得樹的一個複選框的參數
     * @Param: $div: JQuery對象，表示目前選取節點
     * @Param: $i: JQuery對象，表示目前選取的複選框節點
     */
    ...
    /**
     * @Desc: 取得基於傳回參數的樹的複選框參數，選取資料橫向儲存
     */
    ...
    /**
     * @Desc: 在節點後動態綁定fixed和follow條件的工具列
     * @Param: $cite： JQuery對象，表示目前文字節點
     */
    ...
    /**
     * @Desc: toolbar監聽方法
     * @Param: tool: 工具列的具體操作字串
     * @Param: $div: JQuery對象，表示目前操作節點
     */
    ...
    /**
     * @Desc: 載入toolbar中的內容
     * @Param: title: 預設操作中的input輸入框的值
     * @Param: name: 工具列的具體操作字串
     */
        ...
        switch (name) {
            case defaultTool.addTool:
                var addNodeBarDef = [{"label": "目前選取", "name": "nodeTitle", "type": "text", "value": title, "defElem": "nowChoose", "readonly": true},
                    ...
                    {"type": "submit", "value": "確認新增", "defElem": "btn", "filter": "dtree_addNode_" + _this.obj[0].id + "_form"}];
            ...
            case defaultTool.editTool:
                var editNodeBarDef = [{"label": "目前選取", "name": "nodeTitle", "type": "text", "value": title, "defElem": "nowChoose", "readonly": true},
                    ...
                    {"type": "submit", "value": "確認編輯", "defElem": "btn", "filter": "dtree_editNode_" + _this.obj[0].id + "_form"}];
    ...
    /**
     * @Desc: 取得toolbar詳細的標籤資訊
     * @Param: nodeBarContents: 工具列中的資料渲染內容JSON物件
     */
    ...
    /**
     * @Desc: 新增節點後改變節點內容
     * @Param: returnID: 目前需要載入的內容，JSON物件、字串、指定字串（"refresh"）、true
     */
    ...
    /**
     * @Desc: 修改節點後改變節點內容
     * @Param: returnID: 目前需要變更的內容，JSON物件
     */
    ...
    /**
     * @Desc: 載入iframe
     * @Param: $div: JQuery對象，表示當前節點
     * @Param: iframeParam: iframe載入的參數
     */ 
    ...
    /**
     * @Desc: 取得傳遞出去的參數，根據iframe.iframeDefaultRequest、iframe.iframeRequest和node拼出發出請求的參數
     * @Param: nodes: 需要傳遞出去的參數
     */
    ...
    /******************** 資料回呼區域 ********************/
    /**
     * @Desc: 根據具體的id取得基於當前id的div以及對應的其他dom元素
     * @Param: id: 節點的ID值
     */
    ...
    /**
     * @Desc: 取得目前選取節點下一個ul節點 或根節點。為了將新節點放入ul下
     */
    ...
    /**
     * @Desc: 取得傳遞出去的參數，根據defaultRequest、request和node拼出發出請求的參數
     * @Param: nodes：需要傳遞出去的參數
     */
    ...
    /**
     * @Desc:  取得filterParam過濾後的requestParam
...
//                console.log("winHeight = " + winHeight);
    ...
    var dtree = {
        ...
        getCheckbarJsonArrParam: function(dTree){  // 取得複選框選取值
        	if(typeof dTree === "string"){
        		dTree = DTrees[dTree];
        	}
        	if(typeof dTree === "undefined"){
        	...
        	return dTree.getCheckbarJsonArrParam();	// 取得複選框選取值
        },
        ...
        serialize: function(first, param) {  // 序列化JSON物件
